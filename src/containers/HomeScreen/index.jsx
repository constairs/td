...
import { CardItem, Modal, CreateForm, EditForm } from '../../components';
...
export class Home extends React.Component<Object, Object> {
  ...
  onDragEnd = ({ source, destination }: Object) => {
    ...
    const destinationAnswersUpdated =
    ...
    if (
      destinationAnswers.length === destinationAnswersMaxCount &&
      sourceId !== destinationId
    ) {
      const [removedDest] = destinationAnswersUpdated.splice(
        destination.index,
        1
      );
  ...
  get FilteredCards() {
    const {
      state: { cardList, filter: filterParam }
    } = this;
  ...
  render() {
    const {
      state: {
 cardList, editModal, createModal, filter 
},
      FilteredCards,
      onDragEnd
    } = this;
        ...
        <div>
          ...
          >
            <div>
              ...
              <button
                onClick={() => {
                  this.setState({ filter: 'veryImportant' });
                }}
              >
                Very Important
              </button>
